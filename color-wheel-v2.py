...
for band_num in range(1, HALF_SIDE_LENGTH + 1):
	...
	for color_i in range(0, band_size // 8):
		image[color_y, color_x] = ordered_colors_to_place[color_i]
		color_x -= 1
	...
	if placing_progress >= placing_progress_i * TOTAL_COLORS / PLACE_PROGRESS_INCREMENTS:
		...
		if placing_progress_i == PLACE_PROGRESS_INCREMENTS // 4:
			print(f"Pausing to write file color-wheel-v2-1o4...")
			cv2.imwrite(f"color-wheel-v2-1o4.png", image, [cv2.IMWRITE_PNG_COMPRESSION, 9])
			print(f"Resuming placements...")
		elif placing_progress_i == PLACE_PROGRESS_INCREMENTS // 2:
			print(f"Pausing to write file v4-1o2...")
			cv2.imwrite(f"color-wheel-v2-1o2.png", image, [cv2.IMWRITE_PNG_COMPRESSION, 9])
			print(f"Resuming placements...")
		placing_progress_i += 1

print("Writing file...")
cv2.imwrite("color-wheel-v2.png", image, [cv2.IMWRITE_PNG_COMPRESSION, 9])
print("Done")
