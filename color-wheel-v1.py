...
for bright in range(0, MAX_BRIGHTNESS + 1):
...
unplaced_color_datas_count = 0
last_placed_brightness = MAX_BRIGHTNESS
ordered_color_datas_to_place = []
offsets_per_hue = []
monochromes = []
for band_num in range(1, HALF_SIDE_LENGTH + 1):
	band_size = band_num * 8 - 4
	...
	while True:
		...
		#If we have more colors than available spaces, pick out colors by ordering all possible hues by which ones have
		#	been least represented so far
		if unplaced_color_datas_count > band_available_space_count:
			coverage_per_hue = \
				[(hues_placed_so_far[hue] / hue_counts[hue], hue) \
					for hue in unplaced_color_datas_by_hue]
			heapq.heapify(coverage_per_hue)
			add_counts_by_hue = {}
			#One-by-one, mark the hue that has been least represented so far, but don't add the color
			#	because we want an even distribution of saturations
			for _ in range(0, band_available_space_count):
				hue = heapq.heappop(coverage_per_hue)[1]
				add_count = add_counts_by_hue.get(hue, 0) + 1
				add_counts_by_hue[hue] = add_count
				hues_placed_so_far[hue] += 1
				#Re-track the hue if we have colors left for this hue, with an updated representation
				#	ratio
				if add_count < len(unplaced_color_datas_by_hue[hue]):
					heapq.heappush(
						coverage_per_hue, (hues_placed_so_far[hue] / hue_counts[hue], hue))
		...
		#If we have enough colors to use them all up, do so
			...
			for (hue, color_datas) in unplaced_color_datas_by_hue.items():
				...
				hues_placed_so_far[hue] += len(color_datas)
			color_datas_to_place_count += unplaced_color_datas_count
		...
		if last_placed_brightness % 3 == 0:
			...
			if last_placed_brightness == 0:
				break
		#Collect all colors of the next brightness, but don't add them now; we'll do that next loop
		last_placed_brightness -= 1
		color_datas_by_hue = color_datas_by_hue_by_bright[last_placed_brightness]
		for (hue, color_datas) in color_datas_by_hue.items():
			...
			unplaced_color_datas_count += len(color_datas)
	#Resize the colors list to hold all the colors
	for _ in range(len(ordered_color_datas_to_place), color_datas_to_place_count):
		ordered_color_datas_to_place.append(None)
		offsets_per_hue.append(None)
	while len(ordered_color_datas_to_place) > color_datas_to_place_count:
		ordered_color_datas_to_place.pop()
		offsets_per_hue.pop()
	#Insert the colors into the colors list ordered by hue
	offset_total = 0
	ordered_color_i = 0
	for (hue, color_datas_to_place) in sorted(color_datas_to_place_by_hue.items()):
		for _ in range(0, len(color_datas_to_place)):
			offset = ordered_color_i - (hue * color_datas_to_place_count / 6) + 0.5
			offset_total += abs(offset)
			offsets_per_hue[ordered_color_i] = offset
			ordered_color_datas_to_place[ordered_color_i] = hue
			ordered_color_i += 1
	#Insert space for monochrome colors if applicable
	for _ in range(0, len(monochromes)):
		...
		for monochrome_i in range(color_datas_to_place_count - 1, -1, -1):
			...
			if new_offset_total <= offset_best_total \
					and (monochrome_i == 0 \
						or ordered_color_datas_to_place[monochrome_i] \
							!= ordered_color_datas_to_place[monochrome_i - 1]):
		...
		ordered_color_datas_to_place.insert(monochrome_i, None)
	#Replace hues with colors
	color_i = 0
	while color_i < band_size:
		hue = ordered_color_datas_to_place[color_i]
		if hue is None:
			monochrome = monochromes.pop(0)
			ordered_color_datas_to_place[color_i] = (monochrome, monochrome, monochrome)
			color_i += 1
			continue
		color_datas_to_place = color_datas_to_place_by_hue[hue]
		if len(color_datas_to_place) == 1:
			ordered_color_datas_to_place[color_i] = color_datas_to_place[0][1]
			color_i += 1
		else:
			...
			for color_data in color_datas_to_place:
				ordered_color_datas_to_place[color_i_next] = color_data[1]
	...
	#Add all the colors in the colors list to the image
	#We track the position of the next pixel, and each section of the band advances it to the next position each loop
	color_x = HALF_SIDE_LENGTH - 1
	color_y = HALF_SIDE_LENGTH - band_num
	for color_i in range(0, band_size // 8):
		image[color_y, color_x] = ordered_color_datas_to_place[color_i]
		color_x -= 1
	for color_i in range(band_size // 8, 3 * band_size // 8):
		image[color_y, color_x] = ordered_color_datas_to_place[color_i]
		color_y += 1
	for color_i in range(3 * band_size // 8, 5 * band_size // 8):
		image[color_y, color_x] = ordered_color_datas_to_place[color_i]
		color_x += 1
	for color_i in range(5 * band_size // 8, 7 * band_size // 8):
		image[color_y, color_x] = ordered_color_datas_to_place[color_i]
		color_y -= 1
	for color_i in range(7 * band_size // 8, band_size):
		image[color_y, color_x] = ordered_color_datas_to_place[color_i]
		color_x -= 1
	placing_progress += band_size
	if placing_progress >= placing_progress_i * TOTAL_COLORS / PLACE_PROGRESS_INCREMENTS:
		print(
			f"Placed {placing_progress}/{TOTAL_COLORS} colors ({band_num}/{HALF_SIDE_LENGTH} bands," +
			f" {placing_progress_i}/{PLACE_PROGRESS_INCREMENTS})")
		placing_progress_i += 1

print("Writing file...")
cv2.imwrite("color-wheel-v1.png", image, [cv2.IMWRITE_PNG_COMPRESSION, 9])
print("Done")
